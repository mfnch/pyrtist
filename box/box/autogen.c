...
int Auto_Generate_Copier(BoxCmp *c, BoxType t) {
  ...
  if (TS_Is_Empty(ts, t))
    ...
    CmpProc *save_cur_proc = c->cur_proc;
    c->cur_proc = CmpProc_New(c, CMPPROCSTYLE_SUB);
    CmpProc_Set_Prototype(c->cur_proc, 1 /*have_child*/, 1 /*have_parent*/);
    autogenerated = Auto_Generate_Copier_Code(c, t);

    /* If the procedure is not empty, we register it as a conversion for the
     * considered type.
     */
    if (autogenerated) {
      Operation *opn;
      ...
      /* Create the procedure type and obtain its name */
      char *name = Box_SPrintF("copy_%~s", TS_Name_Get(ts, t));
      CmpProc_Set_Alter_Name(c->cur_proc, name);
      BoxMem_Free(name);
      ...
      opn = Operator_Add_Opn(& c->convert, t, BOXTYPE_NONE, t);
      Operation_Set_User_Implem(opn, sym_id);
...
/* Autogeneration of object descriptor for structures */
static int My_Autogen_Struc_Desc(BoxCmp *c, BoxType t) {
#if 0
  size_t initial_proc_size;
  ValueStrucIter vsi_src, vsi_dest;
  Value v_struc_src, v_struc_dest;
...
static void My_Build_Struc_Desc(BoxTS *ts, BoxVM *vm,
  ...
  for (BoxTSStrucIt_Init(ts, & it, t); it.has_more;
    ...
    BoxVMSubObj sub;
    sub.alloc_id = TS_Get_AllocID(ts, vm, t);
...
BoxVMObjDesc *TS_Get_ObjDesc(BoxTS *ts, BoxVM *vm, BoxType t) {
  if (TS_Is_Empty(ts, t))
    /* An empty (size == 0) object does not need an object descriptor! */
    ...
    /* Here we actually build the object descriptor */
    My_Build_Obj_Desc(ts, vm, & bldr, t);

    if (bldr.desc.has.initializer || bldr.desc.has.finalizer
        || bldr.desc.has.copier || bldr.desc.has.mover) {
      /* Create the object description from the builder */
      size_t num_subs = BoxArr_Num_Items(& bldr.subs),
             subs_data_size = sizeof(BoxVMSubObj)*num_subs;
