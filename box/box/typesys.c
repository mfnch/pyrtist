...
BoxVMSymID TS_Procedure_Get_Sym(TS *ts, BoxTypeId p) {
  TSDesc *proc_td;
  proc_td = Type_Ptr(ts, p);
  assert(proc_td->kind == TS_KIND_PROC);
...
static BoxTypeId
My_Procedure_Search(BoxTS *ts, BoxTypeId *expansion_type, BoxTypeId child,
                    BoxCombType comb, BoxTypeId parent) {
...
BoxTypeId
BoxTS_Procedure_Search(TS *ts, BoxTypeId *expansion_type,
                       BoxTypeId child, BoxCombType comb, BoxTypeId parent,
                       TSSearchMode mode) {
  BoxTypeId previous_parent;
