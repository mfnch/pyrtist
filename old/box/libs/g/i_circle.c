...
BoxTask circle_color(BoxVMX *vmp) {
...
BoxTask circle_gradient(BoxVMX *vmp) {
  return x_gradient(vmp);
}

BoxTask circle_begin(BoxVMX *vmp) {
  SUBTYPE_OF_WINDOW(vmp, w);
...
static BoxTask _circle_point(Window *w, BoxPoint *center) {
  if (w->circle.got.center == GOT_NOW) {
    if (!w->circle.got.radius_a) {
      BoxReal dx = w->circle.center.x - center->x,
           dy = w->circle.center.y - center->y;
      BoxReal radius = sqrt(dx*dx + dy*dy);
...
BoxTask circle_point(BoxVMX *vmp) {
  SUBTYPE_OF_WINDOW(vmp, w);
  BoxPoint *center = BOX_VM_ARG1_PTR(vmp, BoxPoint);
  return _circle_point(w, center);
}

BoxTask circle_real(BoxVMX *vmp) {
  SUBTYPE_OF_WINDOW(vmp, w);
  BoxReal radius = BOX_VM_ARG1(vmp, BoxReal);
...
BoxTask circle_style(BoxVMX *vmp) {
...
static BoxTask _circle_draw(Window *w, DrawWhen when) {
...
BoxTask circle_pause(BoxVMX *vmp) {
  SUBTYPE_OF_WINDOW(vmp, w);
  return _circle_draw(w, DRAW_WHEN_PAUSE);
}

BoxTask circle_end(BoxVMX *vmp) {
  SUBTYPE_OF_WINDOW(vmp, w);
  BoxTask t = _circle_draw(w, DRAW_WHEN_END);
...
static BoxTask _add_from_pl(BoxInt index, char *name, void *object, void *data) {
  ...
  BoxPoint *center = (BoxPoint *) object;
...
BoxTask circle_pointlist(BoxVMX *vmp) {
